class Ball      // These data members of a Ball below are automatically filled in for you from the text file, due to code that's already present in Ray_Tracer::parse_line().
{
  constructor(               position, size, color, k_a, k_d, k_s, n, k_r, k_refract, refract_index   )
    { Object.assign( this, { position, size, color, k_a, k_d, k_s, n, k_r, k_refract, refract_index } )
      ...
      //scale at origin, then translate away from origin(scale won't affect translate)
  ...
  intersect( ray, existing_intersection, minimum_dist )
    { // TODO:  Given a ray, check if this Ball is in its path.  Its first argument is the ray, a key/value object with an origin and a direction as keys.  The next argument
      ...
      // origin is 4-d, should make it 3
      let A = c_prime.dot(c_prime);
      let B = s_prime.dot(c_prime);
      let C = s_prime.to3().dot(s_prime.to3()) - 1;
      let discriminant = B*B - A*C;

      if( discriminant>0 )
      { let farther_t = (-B + Math.sqrt(discriminant))/A;//t1
        let closer_t = (-B - Math.sqrt(discriminant))/A;//t2
        let direction = 1;

        // if(closer_t<minimum_dist || closer_t >=existing_intersection.distance)
        // {
        //   //father_t is closer
        //   if(farther_t < closer_t)
        //   {
        //     closer_t = farther_t;
        //     direction = -1;
        //   }
        //   //closer_t is closer
        //   else {
        //     return existing_intersection;//didn't assign new intersection
        //   }
        // }


        if (closer_t < minimum_dist || closer_t > farther_t || closer_t >= existing_intersection.distance)
        {
  			  closer_t = farther_t;
  			  direction = -1; 
		    }
        ...
        //compare new closer_t with existing_intersection
        if((closer_t>minimum_dist)&&(closer_t<existing_intersection.distance))
        {
          //assign new intersection
          existing_intersection.ball = this;
          existing_intersection.distance = closer_t;
          let normal_unnormalized = this.inverse.transposed().times(s_prime.plus(c_prime.times(closer_t)).to4(false));
          existing_intersection.normal = normal_unnormalized.normalized().times(direction);
          return existing_intersection;
        }
      }
      //no intersection, no update
      return existing_intersection;
    }
}
...
{                                           // Textures and a hidden canvas are utilized to paint the pixel colors onto a Square somewhere that WebGL can show.
  ...
  trace( ray, color_remaining, is_primary, is_shadow_ray = false, light_to_check = null )
    { // TODO:  Given a ray, return the color in that ray's path.  The ray either originates from the camera itself or from a secondary reflection or refraction off of a
      ...
      //get which ball is hit, best...

      for( let b of this.balls ) closest_intersection = b.intersect( ray, closest_intersection, is_primary ? 1 : .0001 );

      if( !closest_intersection.ball ) return this.color_missed_ray( ray ); //if ball color same as background
      ...
      let sphere_color = closest_intersection.ball.color;//(3)
      let intersection_point = ray.origin.plus(ray.dir.times(closest_intersection.distance));//(3)
      let normal = closest_intersection.normal.to3().normalized();
      let k_a = closest_intersection.ball.k_a;//(3)
      var k_d = closest_intersection.ball.k_d;//(3)
      var k_s = closest_intersection.ball.k_s;//(3)
      let diffuse_allLights = Color.of(0,0,0);
      let specular_allLights = Color.of(0,0,0);
      let Blinn_illumination_color = Color.of(0,0,0);
      let total_illumination_color = Color.of(0,0,0);//including reflection and refraction
      let ambient_allLights = sphere_color.times(k_a);
      let light_intersection = { distance: Number.POSITIVE_INFINITY, ball: null, normal: null };
      let light_ray = {origin:intersection_point.to4(true), dir:Vec.of(0,0,0)} ;
      let v = ray.origin.to3().minus(intersection_point).normalized();
      let n = closest_intersection.ball.n;
      let whiteColor = [1, 1, 1];

      //diffuse & specular
      for (var i=0;i<this.lights.length;i++){
         let blocked = false;
         let sameDir_n_l = true;
         light_ray.origin = intersection_point.to4(true);
         light_ray.dir = this.lights[i].position.to3().minus(intersection_point).to4(false).normalized();
         let l = this.lights[i].position.to3().minus(intersection_point).normalized();
         let h = l.plus(v).normalized();
         let light_color = this.lights[i].color.to3();
         for( let temp_ball of this.balls )
         {
           light_intersection = temp_ball.intersect(light_ray,light_intersection, is_primary ? 1 : .0001 );
           if(light_intersection.ball) blocked = true;
         }
         //blocked means be blocked by other project
         //second part is blocked by innner light with another intersect point on this ball,
         //it doesn't really blocked
         if(!blocked || (this.trace(light_ray,color_remaining,false,true,this.lights[i])=="No shadow here!"))
         {
           //if (l.dot(normal)<=0) sameDir_n_l = false;
           if (l.dot(normal)>0){
				         diffuse_allLights = diffuse_allLights.plus(light_color.times(l.dot(normal)).times(k_d).mult_pairs(sphere_color));
                 specular_allLights = specular_allLights.plus(light_color.times(Math.pow(h.dot(normal),n)).times(k_s));
			     }
         }
       }
       //add together
       Blinn_illumination_color = diffuse_allLights.plus(specular_allLights).plus(ambient_allLights);
       let Blinn_illumination_color_plus = Blinn_illumination_color.minus(whiteColor);
       let Blinn_illumination_color_plus2 = Vec.of(1,1,1).minus(Blinn_illumination_color);
       //reflections
       let reflection_ray = {origin:intersection_point.to4(true), dir: normal.times(2 * normal.dot(v)).minus(v).normalized().to4(false)};
       let color_remaining_reflection = color_remaining.times(closest_intersection.ball.k_r).mult_pairs(Blinn_illumination_color_plus);
       let all_light_color_reflection = this.trace(reflection_ray, color_remaining_reflection, false, false,light_to_check);
       let illumination_reflection = all_light_color_reflection.times(closest_intersection.ball.k_r).mult_pairs(Blinn_illumination_color_plus2).to3();
       //refractions
       let refraction_ratio = closest_intersection.ball.refract_index;
       let refraction_l = ray.dir.to3().normalized();
       let refraction_c = normal.dot(refraction_l)*(-1);
       let refraction_y = refraction_c*(refraction_ratio)-(Math.sqrt(1-refraction_ratio*refraction_ratio*(1-refraction_c*refraction_c)));
       let refraction_out_ray = refraction_l.times(refraction_ratio).plus(normal.times(refraction_y)).to4(false);
       let refraction_ray = {origin:intersection_point.to4(true), dir:refraction_out_ray};
       let color_remaining_refraction = color_remaining.times(closest_intersection.ball.k_refract).mult_pairs(Blinn_illumination_color_plus);
       let all_light_color_refraction= this.trace(refraction_ray, color_remaining_refraction, false, false,light_to_check);
       let illumination_refraction = all_light_color_refraction.times(closest_intersection.ball.k_refract).mult_pairs(Blinn_illumination_color_plus2).to3();

       total_illumination_color = Blinn_illumination_color.plus(illumination_reflection).plus(illumination_refraction);
       return total_illumination_color;
    }
constructor( context )
    { super( context );
